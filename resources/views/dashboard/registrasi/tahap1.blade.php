            ...
            <div class="row">
                ...
                <div class="col-sm-12 col-md-8">
                    <div class="card card-outline-info">
                        ...
                        <div class="card-body">
                            <form method="POST" action="/registrasi/tahap1" enctype="multipart/form-data">
                                <div class="row justify-content-center">
                                    <div class="col-sm-12">
                                        @csrf
                                        <div class="form-group row">
                                            <label class="col-sm-4">Jenis Mahasiswa</label>
                                            <div class="col-sm-8">
                                                <select class="form-control" name="jenis_mhs" id="selectJenisMhs">
                                                    @if(Auth::user()->jenis_mhs == 1)
                                                        <option value="1" selected>Regular</option>
                                                        <option value="2">Ampulen</option>
                                                        <option value="3">Pindahan</option>
                                                    @elseif(Auth::user()->jenis_mhs == 2)
                                                        <option value="1">Regular</option>
                                                        <option value="2" selected>Ampulen</option>
                                                        <option value="3">Pindahan</option>
                                                    @elseif(Auth::user()->jenis_mhs == 3)
                                                        <option value="1">Regular</option>
                                                        <option value="2">Ampulen</option>
                                                        <option value="3" selected>Pindahan</option>
                                                    @endif
                                                </select>
                                                @if($errors->has('jenis_mhs'))
                                                    <div class="form-control-feedback text-danger">{{ $errors->first('jenis_mhs') }}</div>
                                                @endif
                                            </div>
                                        </div>
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-8">
                                                <select class="form-control" name="jenjangprodi_id" id="selectJenjangProdi">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-8">
                                                <select name="cama_idagama" class="form-control" required>
                                                    @if(count($agama) > 0)
                                                        @foreach($agama as $e_agama)
                                                            @if($e_agama->agama_id == Auth::user()->cama_idagama)
                                                                <option value="{{ $e_agama->agama_id }}" selected>{{ $e_agama->agama }}</option>
                                                            @else
                                                                <option value="{{ $e_agama->agama_id }}">{{ $e_agama->agama }}</option>
                                                            @endif
                                                        @endforeach
                                                    @endif
                                                </select>
                                        ...
                                        <hr>
                                        {{-- Khusus Ampulen --}}
                                        <div id="collapAmpulen">
                                            <div class="form-group row">
                                                <label class="col-sm-4">Asal Program Studi</label>
                                                <div class="col-sm-8">
                                                    <select class="form-control selectpicker" data-live-search="on" name="asal_prodi_id">
                                                        @if(count($fakultas))
                                                            @foreach ($fakultas as $e_fakultas)
                                                                @if(count($e_fakultas->prodi) > 0)
                                                                    <optgroup label="{{ $e_fakultas->nama_fakultas }}">
                                                                        @foreach($e_fakultas->prodi as $e_prodi)
                                                                            <option value="{{ $e_prodi->prodi_id }}"
                                                                                @if($e_prodi->prodi_id == Auth::user()->asal_prodi_id)
                                                                                    selected
                                                                                @endif 
                                                                            >
                                                                                {{ $e_prodi->nama_prodi }}
                                                    ...
                                                    @if($errors->has('asal_prodi_id'))
                                                        <div class="form-control-feedback text-danger">{{ $errors->first('asal_prodi_id') }}</div>
                                                    @endif
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                <label class="col-sm-4">Asal Program</label>
                                                <div class="col-sm-8">
                                                    <select class="form-control" name="asal_program_id">
                                                        @if(count($program))
                                                            @foreach ($program as $e_program)
                                                                <option value="{{ $e_program->program_id }}"
                                                                    @if($e_program->program_id == Auth::user()->asal_program_id)
                                                                        selected
                                                                    @endif 
                                                                >
                                                                    {{ $e_program->nama_program }}
                                                    ...
                                                    @if($errors->has('asal_program_id'))
                                                        <div class="form-control-feedback text-danger">{{ $errors->first('asal_program_id') }}</div>
                                                    @endif
                                                </div>
                                            </div>
                                            <hr>
                                        ...
                                        {{-- Khusus Pindahan --}}
                                        <div id="collapPindahan">
                                            <div class="form-group row">
                                                <label class="col-sm-4">Asal Universitas</label>
                                                <div class="col-sm-8">
                                                    <select name="asal_univ_id" id="universitas" class="form-control selectpicker" data-live-search="true" autocomplete="off" required>
                                                        <option value="0">Belum Terdaftar</option>
                                                        @if(count($univ) > 0)
                                                            @foreach($univ as $e_univ)
                                                                @if($e_univ->univ_id == Auth::user()->asal_univ_id)
                                                                    <option value="{{ $e_univ->univ_id }}" selected>{{ $e_univ->nama_univ }}</option>
                                                                @else
                                                                    <option value="{{ $e_univ->univ_id }}">{{ $e_univ->nama_univ }}</option>
                                                                @endif
                                                            @endforeach
                                                        @endif
                                                    </select>
                                                    @if($errors->has('asal_univ_id'))
                                                        <div class="form-control-feedback text-danger">{{ $errors->first('asal_univ_id') }}</div>
                                                    @endif
                                                </div>
                                            </div>
                                            <div class="form-group row" id="desUniv">
                                                <label class="col-sm-4"></label>
                                                <div class="col-sm-8">
                                                    <textarea name="asal_univ_des" rows="3" class="form-control" placeholder="Masukan nama Universitas Anda jika belum terdaftar diatas">{{ Auth::user()->asal_univ_des }}</textarea>
                                                    @if($errors->has('asal_univ_des'))
                                                        <div class="form-control-feedback text-danger">{{ $errors->first('asal_univ_des') }}</div>
                                                    @endif
                                                </div>
                                            </div>
                                            <hr>
                                        ...
                                        <div class="form-group row">
                                            <label class="col-sm-4">Asal SMA/SMK</label>
                                            <div class="col-sm-8">
